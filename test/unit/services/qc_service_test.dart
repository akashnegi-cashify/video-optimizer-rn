  ...
  group('QcService', () {
    ...
    group('constructor', () {
      ...
      test('should create instance with addAuthorization as true', () {
        // Arrange & Act
        final service = QcService(addAuthorization: true);
      ...
      test('should create instance with addAuthorization as false when specified', () {
        // Arrange & Act
        final service = QcService(addAuthorization: false);
    ...
    group('getServiceGroup', () {
      test('should return TRCServiceGroups.qcConsole', () {
        // Act
        final result = sut.getServiceGroup();

        // Assert
        expect(result, TRCServiceGroups.qcConsole);
      });

      test('should return qcConsole regardless of addAuthorization value', () {
        // Arrange
        final serviceWithAuth = QcService(addAuthorization: true);
        final serviceWithoutAuth = QcService(addAuthorization: false);

        // Act & Assert
        expect(serviceWithAuth.getServiceGroup(), TRCServiceGroups.qcConsole);
        expect(serviceWithoutAuth.getServiceGroup(), TRCServiceGroups.qcConsole);
      });

      test('service group value should be "qc-console"', () {
        // Act
        final result = sut.getServiceGroup();

        // Assert
        expect(result.value, 'qc-console');
      });
    });
    ...
    group('getHeaders', () {
      test('should return SSO token headers when isToAddAuth is null (uses default)', () {
        ...
        // Since isToAddUserAuth() returns true by default, headers should include SSO token
    ...
    group('multiple instances', () {
      test('should have independent authorization settings', () {
        // Arrange
        final service1 = QcService(addAuthorization: true);
        final service2 = QcService(addAuthorization: false);
      ...
      test('different instances should return same service group', () {
        // Arrange
        final service1 = QcService();
        final service2 = QcService(addAuthorization: true);
        final service3 = QcService(addAuthorization: false);

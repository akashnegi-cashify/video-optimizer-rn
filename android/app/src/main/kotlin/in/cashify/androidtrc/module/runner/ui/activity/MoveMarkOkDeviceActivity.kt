...
import `in`.cashify.androidtrc.databinding.ActivityMoveMarkOkDeviceBinding
import `in`.cashify.androidtrc.module.runner.api.response.marked_ok_device.MoveToMarkOkTray
import `in`.cashify.androidtrc.module.runner.data.MoveMarkOkDeviceViewModel
 ...
 */
class MoveMarkOkDeviceActivity : BaseActivity(), BarcodeScannerFragment.BarcodeScannerListener {

    override fun getLayoutResId(): Int {
        return R.layout.activity_move_mark_ok_device
    }

    override fun onBarcodeScanned(barcode: String) {
        scannerFragment.pauseScanner()
        viewModel?.moveToMarkOK(mTrayBarcode, barcode, object : OnResult<MoveToMarkOkTray> {
            override fun onResultAvailable(data: MoveToMarkOkTray) {
                if (data.isSuccess) {
                    showDialog(
                        "Alert",
    ...
    lateinit var binding: ActivityMoveMarkOkDeviceBinding
    var viewModel: MoveMarkOkDeviceViewModel? = null
    ...
    override fun create(savedInstanceState: Bundle?, mainContainer: LinearLayout, addToParent: Boolean) {
        ...
        viewModel = ViewModelProviders.of(this, factory).get(MoveMarkOkDeviceViewModel::class.java)
        viewModel?.activityListener = this
        binding.lifecycleOwner = this
        setTitle("Move To Mark Ok")
        AnalyticsEventHelper.fireScreenEvent(this, AnalyticsController.AnalyticEventKey.EVENT_ONCREATE, AnalyticsController.AnalyticScreen.SCREEN_RUNNER_MOVE_TO_MARK_OK, null, null, true)

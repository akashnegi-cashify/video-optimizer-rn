 ...
 */
class VirtualStoreViewModel @Inject constructor() : BaseViewModel()
{
    ...
    fun saveVirtualStoreIn(
         deviceBarcode: String?,
        onResult: OnResult<VirtualStoreResponse>
    ){
        activityListener?.showLoading(true)
        val service =
            UserModuleService<StorageMangerAPi, VirtualStoreResponse>(
                StorageMangerAPi::class.java,
                mResourceProvider.mContext
            )
        service.execute(object :
            SessionBaseAPICallback<StorageMangerAPi, VirtualStoreResponse>(activityListener) {
            override fun onSuccess(response: VirtualStoreResponse, rawResponse: Response) {
                onResult.onResultAvailable(response)
            }
            ...
            override fun getAPIAsync(api: StorageMangerAPi): Deferred<retrofit2.Response<VirtualStoreResponse>> {
                return api.getVirtualStoreIn(deviceBarcode)
            }
    ...
    fun getVirtualStoreOut(
        ...
        service.execute(object :
            ...
            override fun getAPIAsync(api: StorageMangerAPi): Deferred<retrofit2.Response<VirtualStoreResponse>> {
                return api.getVirtualStoreOut(deviceBarcode)
            }
